...
const SignupScreen = ({ navigation, route }) => {
  ...
  const registerAndGoToLogin = async (payload) => {
    ...
    try {
      ...
      if (res.success) {
        if (profilePhoto?.uri && res.data?.memberId) {
          await uploadProfilePhotoForMember(res.data.memberId, profilePhoto.uri);
        }
        Alert.alert(
          'Registration Successful! 🎉',
          'You can pay within 3 days to keep your account active.',
